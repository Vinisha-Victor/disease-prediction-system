...
async def train_system(config: TrainingConfig):
    ...
    try:
        ...
        training_status["ga_metrics"] = results["ga_metrics"]
        training_status["pso_metrics"] = results["pso_metrics"]
        training_status["de_metrics"] = results["de_metrics"]
...
def export_features():
    ...
    csv_content = "feature,selected\n"
    for feat in selected:
        csv_content += f"{feat},1\n"
    
    return Response(
        content=csv_content,
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=selected_features_ensemble.csv"}
    )
