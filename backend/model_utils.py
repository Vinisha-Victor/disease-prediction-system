...
def fitness_function(mask, gen, total_gens, prev_best_mask=None, alpha=0.0005, beta=0.08):
    """
    Fitness function with dynamic weights, stability, and incremental evaluation
    Alpha=0.0005 balances accuracy (>90%) with reasonable feature count (8-10 features)
    """
    global X_train, y_train, X_val, y_val
...
def random_mask(n, p=0.35):
    """Generate random binary mask - balanced to start with ~35% features for optimal convergence"""
...
def preprocess_dataset(df, filename):
    ...
    # Special fix for Diabetes dataset (optimized)
    ...
    # Convert to numeric and handle NaN (optimized)
    ...
    # Fix Heart Disease dataset: invert labels
...
def evaluate_mask_metrics(mask, mask_name="Mask"):
...
def train_models(gens=20, pop=20):
    ...
    ga_metrics = evaluate_mask_metrics(ga_mask, "GA")
    pso_metrics = evaluate_mask_metrics(pso_mask, "PSO")
    de_metrics = evaluate_mask_metrics(de_mask, "DE")
